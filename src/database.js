...
const db = {
	...
	reward: {
		...
		async add(info, admin) {
			await query(
				"INSERT INTO tblConservationAdminStock (casName, casDescription, casImage, casStockAmount, casRandValue, casCoinValue, casVerified, tblAdminUser_admID) VALUES (?, ?, ?, ?, ?, 0, 0, ?)",
				[info.name, info.description, Buffer.from(info.image, "base64"), info.amount, info.randValue, admin]
			);
		},

		async remove(id) {
			await query(
				"DELETE FROM tblConservationAdminStock WHERE casID = ?",
				[id]
			);
		},
		...
		async find(id) {
			const results = await query(
				"SELECT casID FROM tblConservationAdminStock WHERE casID = ?",
				[id]
			);
			if(results.length) {
		...
		async getAdmin(id) {
			const results = await query(
				"SELECT tblAdminUser_admID FROM tblConservationAdminStock WHERE casID = ?",
				[id]
			);
			return results[0].tblAdminUser_admID;
		},
		...
		async list() {
			const results = await query(
				"SELECT casID, casName, casDescription, casImage, casStockAmount, casRandValue, casCoinValue FROM tblConservationAdminStock WHERE casVerified = 1"
			);
			const rewards = [];
		...
		async listOwned(admin) {
			const results = await query(
				"SELECT casID, casName, casDescription, casImage, casStockAmount, casRandValue, casCoinValue, casVerified FROM tblConservationAdminStock WHERE tblAdminUser_admID = ?",
				[admin]
			);
			const rewards = [];
			for(let reward of results) {
		...
		async listNew() {
			const results = await query(
				"SELECT casID, casName, casDescription, casImage, casStockAmount, casRandValue FROM tblConservationAdminStock WHERE casVerified = 0"
			);
			const rewards = [];
			for(let reward of results) {
				rewards.push({
		...
		async verifyCoinValue(id, coinValue) {
			await query(
				"UPDATE tblConservationAdminStock SET casCoinValue = ?, casVerified = 1 WHERE casID = ?",
				[coinValue, id]
			);
		}
	}
